...
function PlayPage() {
  ...
  const OverlayRenderer = useMemo(() => {
    const Component = ({
      ...
      if (sceneKey === "CharacterCreate")
        return <CharacterCreateOverlay game={game} />;
      if (sceneKey === "MainScene")
        return (
          <MainHudOverlay changeScene={(s) => setInitialScene(s as SceneKey)} />
        );
      return null;
    };
  ...
  if (!initialScene) {
    return (
      <div className="flex h-[calc(100vh-60px)] items-center justify-center">
        <div className="rounded-xl bg-black/60 text-white px-4 py-2">
          Loading…
        </div>
      </div>
    );
